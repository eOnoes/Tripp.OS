...
export type AgentBusTraceEventType = string;
...
export async function writeTaskPacket(
  ...
  await ensureDir(dir);
  const fileName = `task-${packet.packetId ?? generatePacketId()}.json`;
  const filePath = path.join(dir, fileName);
  const data = { ...packet, writtenAt: new Date().toISOString() };
...
export async function writeResultPacket(
  ...
  await ensureDir(dir);
  const fileName = `result-${packet.packetId ?? generatePacketId()}.json`;
  const filePath = path.join(dir, fileName);
  const data = { ...packet, writtenAt: new Date().toISOString() };
  await fs.writeFile(filePath, JSON.stringify(data, null, 2) + "\n", "utf-8");
  return filePath;
